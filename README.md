# Game Compass (게임 나침반)

사용자의 취향과 플레이 상황을 반영해 **지금 나에게 맞는 게임과 게임 모드**를 추천하는 AI 서비스입니다.

## 문제 정의

게임/모드 선택지는 많지만, 인기순 추천만으로는 개인 취향과 현재 상황(혼자/친구와, 짧은 시간/긴 시간, 경쟁/힐링 등)을 충분히 반영하기 어렵습니다.

## 핵심 기능 (MVP)

1. **취향 기반 맞춤 추천**: 간단한 질문 응답을 통해 선호 성향 파악
2. **상황 반영 추천**: 현재 플레이 상황(시간, 인원, 난이도 선호 등) 입력 반영
3. **추천 이유 설명**: 왜 이 게임/모드가 맞는지 AI가 이해 가능한 문장으로 설명

## 추천 시스템 접근 방식

- **후보 추출**: 태그 기반 필터링/매칭(장르, 난이도, 플레이 시간, 멀티 여부, 분위기 등)
- **설명 생성**: LLM을 이용해 개인화된 추천 이유 생성
- **핵심 구조**: `질문 응답 + 태그 매칭 + LLM 설명`

## Kanban Board

### C1. 사용자 성향 분석

- [ ] 취향 질문 항목 정의하기
- [ ] 플레이 스타일 태그 설계하기
- [ ] 상황 질문(혼자/친구, 짧게/길게, 경쟁/힐링) 정리하기
- [ ] 사용자 답변을 구조화된 프로필로 변환하기
- [ ] 답변 누락 시 기본값 규칙 정하기

### C2. 게임/모드 조회

- [ ] 게임/모드 분류 체계 정의하기
- [ ] 마인크래프트 모드 장르를 기능 기준으로 재분류하기
- [ ] CurseForge/Modrinth 참고용 데이터 수집 기준 정하기
- [ ] 게임/모드 메타데이터 CSV 또는 JSON으로 정리하기
- [ ] 태그 기반 후보 조회 로직 만들기
- [ ] 추천 제외 조건과 우선순위 규칙 정하기

### C3. 답변 구조

- [ ] 추천 결과 응답 포맷 정의하기
- [ ] 추천 이유를 한 줄 요약 + 상세 설명으로 나누기
- [ ] 추천 결과를 카드 형태로 보여주는 UI 만들기
- [ ] 재추천/수정 질문 흐름 설계하기
- [ ] 추천 결과 저장 또는 복사 기능 검토하기
- [ ] 응답 템플릿을 LLM 프롬프트와 연결하기

### C4. 검증

- [ ] 실제 사용자 시나리오 5~10개로 추천 품질 점검하기
- [ ] 잘못 분류된 항목 기준으로 태그 보정하기
- [ ] MVP 범위에 맞게 대상 게임/모드 수 제한하기

## 기술 스택 (제안)

- **Frontend**: React + TypeScript + Vite (또는 Streamlit)
- **Backend**: FastAPI (Python)
- **Data**: CSV/JSON 기반 게임·모드 메타데이터, PostgreSQL(초기 MVP는 SQLite 가능)
- **AI**: OpenAI API(또는 동급 LLM API)
- **(선택) 검색 고도화**: pgvector 또는 FAISS
- **Deployment**: Vercel(Frontend), Render/Railway(Backend)

## 데이터 소스 예시

- Steam 게임 정보
- 장르/태그 데이터
- 공개 모드 설명 데이터(수집 가능한 범위)
- 팀 자체 정리 데이터셋(CSV/JSON)

## 기대 효과

- 게임/모드 선택 시간 단축
- 추천 실패 확률 감소
- 사용자 취향 이해도 향상

## 확장 방향

게임 추천에서 시작해 영화/음악/웹툰 등 취향 기반 추천 서비스로 확장할 수 있습니다.
